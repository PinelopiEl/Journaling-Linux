			    ...
			    struct msdos_dir_slot *slots, int *nr_slots)
{
	...
	*nr_slots = usize / 13;
	for (ps = slots, i = *nr_slots; i > 0; i--, ps++) {
		...
		//############################################## PRINT ########################################
		printk(KERN_INFO " EIMASTE STO vfat_build_slots STO NAMEI_VFAT\n");
		sprintf(obuffermoy, "ps->id =%u,ps->attr = %u,ps->reserved =%u, ps->alias_checksum=%u,ps->start=%u, ps->name5_10 =%u,ps->name11_12 = %u  \n", ps->id,ps->attr,ps->reserved,ps->alias_checksum,ps->start,ps->name5_10,ps->name11_12 );
...
shortname:
	...
	//############################################## PRINT gia msdos_dir_entry ########################################
	printk(KERN_INFO " EIMASTE STO vfat_build_slots sto namei_vfat\n");
	sprintf(obuffermoy, "de->name =%s\n", de->name);
	sprintf(obuffermoy, "de->attr =%u\n", de->attr);
	sprintf(obuffermoy, "de->lcase =%u\n",de->lcase);
	sprintf(obuffermoy, "de->time =%u\n",de->time);
	sprintf(obuffermoy, "de->date =%u\n",de->date);
	sprintf(obuffermoy, "de->ctime_cs =%u\n",de->ctime_cs);
	printk(KERN_INFO "O BUFFER EXEI MESA = %s\n", obuffermoy);
				  ...
				  unsigned int flags)
{
	...
	printk(KERN_INFO "reached vfat_lookup");
		       ...
		       bool excl)
{
	...
	printk(KERN_INFO "reached vfat_create");
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	printk(KERN_INFO "reached vfat_rmdir");
...
static int vfat_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	printk(KERN_INFO "reached vfat_mkdir");
		       ...
		       unsigned int flags)
{
	...
	printk(KERN_INFO "reached vfat_rename");
